...
const QRScanner = () => {
  ...
  const fetchBookingDetails = async (bookingId) => {
      ...
      try {
          ...
          const response = await axios.get(`http://localhost:8080/api/qrcheck/bookingDetail/${bookingId}`, {
              headers: {
  ...
  const fetchTransactionData = async (bookingId) => {
      ...
      setError(null);
      try {
          const response = await axios.get(`http://localhost:8080/api/transactions/${bookingId}`, {
              headers: {
                  Authorization: `Bearer ${token}`,
                  Accept: '*/*'
              }
...
const filteredBookingDetails = Array.isArray(bookingDetails) ? bookingDetails.filter(detail => {
                                  ...
                                  <tbody>
                                      {filteredBookingDetails.map((detail, index) => (
                                          <tr key={index}>
                                              ...
                                              <td>
                                                  {detail.status !== 'CHECKED_IN' && (
                                                      <button className="btn btn-custom-primary" onClick={() => handleCheckIn(detail.bookingDetailsId)}>
                                                          Check-In
                                                      </button>
                                                  )}
                                              </td>
          ...
          <div className="col-md-4">
              <div className="card mb-4">
                  <div className="card-body">
                      ...
                      />
                      <button className="btn btn-custom-secondary" onClick={refreshScanner}>Refresh Scanner</button>
                      ...
                      <div className="card-body">
                          ...
                          <form>
                              ...
                              {transactionData.status === 'DEPOSITED' && (
                                  <button type="button" className="btn btn-custom-success" onClick={() => updateTransactionStatus(transactionData.transactionId)}>
                                      Update Status
                                  </button>
                              )}
                          </form>
